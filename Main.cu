//Edited Gell File to exclude dead cells from the simmulation
//Emma Chaney (6/11/2025)
...
#include <stdlib.h>
#include <stdio.h>
#include <fstream> //EKC = file io
#include <iomanip> //EKC = set fill
...
#define show_time_every_iter   true  // if false, only show every time after Save_data_gap
...
int main(void)
{
	...
	//std::string datapath = "D:/Gell_Data/HDS/test/"; //COMMENTED THIS OUT-EKC
	std::string datapath = "./data/";//UNCOMMENTED THIS -EKC
	...
	savecsv_cell(CpuCell, currentnum, datapath + "Gell_000.csv");
	...
	//Tracking cell growth EKC
	std::ofstream of;
	of.open("./data/cellgrowth.csv");
	of << "Wall(s),Sim Time(m),Itt,Cnt,Dur(s),t/cell(s)" << std::endl;

	//Tracking the maximum cell count. 
	//Acts as a built in trigger within the for loop: stop if X iterations have occurred with no change to the maximum number of cells.
	int curr_max_cell_cnt = 0;
	int itt_since_max_cnt_update = 0;

	/*+++++++++++++++++++++++++++MAIN FOR LOOP+++++++++++++++++++++++++++*/
	//Wall Time based for loop:
	//for (float current_cell_time = 0.f; (current_cell_time < Max_Simulation_Time) && ((clock() - Initstart) < Max_Wall_Time * CLOCKS_PER_SEC ); current_cell_time += Biology_dt) {
	
	//Stability based for loop:
	for(float current_cell_time = 0.f; (itt_since_max_cnt_update < Itts_Till_Stable) && (current_cell_time < Max_Simulation_Time); current_cell_time += Biology_dt) {
		SimuIter++;
		...
		//****************************MAX CELL COUNT TRACKING*******************//
		if(currentnum > curr_max_cell_cnt)
		{
			curr_max_cell_cnt = currentnum;
			itt_since_max_cnt_update = 0;
		}
		else {itt_since_max_cnt_update++;}
		...
		if (show_time_every_iter) {
			double walltime = (double)(Iterend - Simustart) / CLOCKS_PER_SEC;
			double sPerCell = (double)iterduration/currentnum;
			std::cout << "Simulation Iter " << SimuIter << " Finished, takes " << iterduration << "s" << std::endl;
			std::cout << "Cell Num: " << currentnum << std::endl;
			std::cout << "Curr Max: " << curr_max_cell_cnt << " for " << itt_since_max_cnt_update << " itts" << std::endl;
			std::cout << "Accumulative Time " << walltime << "s" << std::endl;
			std::cout << std::endl;

			of << walltime << "," << current_cell_time << "," << SimuIter << "," << currentnum << "," << iterduration << "," << sPerCell << std::endl;
		}
		if (showandsave) {
			...
			if (save_intermediate_Cell) {
				...
				filename << datapath << "Gell_" << std::setw(3) << std::setfill('0') << filesaveidx << ".csv";
				savecsv_cell(CpuCell, currentnum, filename.str());
			}
			...
			showGPUinfo(true);
			double walltime = (double)(showsimutimeend - Simustart) / CLOCKS_PER_SEC;
			...
			//Tracking cell growth EKC
			//else {of << (double)(showsimutimeend - Simustart) / CLOCKS_PER_SEC << "," <<SimuIter << "," << currentnum << "," <<  << std::endl;}
	...
	if (!save_intermediate_Cell) {
		CpuCell = GpuCell;
		savecsv_cell(CpuCell, currentnum, datapath + "Simuend.csv");
	}
	if (!save_intermediate_Mesh) {
		O2Mesh.move_to_CPU();
		O2Mesh.Slice_to_CPU();
	...
	savecsv_array(cellnumrecord, simutimerecord, datapath + "Time_record.csv");
	savecsv_gaparray(cellnumrecord, gapsimutimerecord, datapath + "GapTime_record.csv");
