...
////[0 = quiescent] [1 = premitotic] [2 = postmitotic] [3=apoptotic] [4 = early necrotic] [5 = late necrotic]
	...
	__host__ __device__ bool Phase_update_ki67adv(float rand, float O2, int currentnum);
...
bool Cell::Phase_update_ki67adv(float rand, float O2, int currentnum) {
	...
	apoptosis_rate = Tumor_Apoptosis_rate;
	necrosis_rate = Tumor_Necrosis_Rate;
	//dN/dt = rN(1 - N/K)
	proliferation_rate = Tumor_Proliferation_Rate *(1 - float(currentnum)/Target_Cell_Count); //took out * current num.
